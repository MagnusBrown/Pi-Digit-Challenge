...
export function importAllData(data){if(!data||!Array.isArray(data.runs))throw new Error("Ugyldig datafil");localStorage.setItem(RUNS_KEY,JSON.stringify(data.runs.slice(0,500)));if(data.playerName)localStorage.setItem("pi-player-name",data.playerName);if(data.profileCreatedAt)localStorage.setItem("pi-profile-created-at",data.profileCreatedAt);localStorage.setItem("pi-achievements-v1",JSON.stringify(data.achievements||{}));localStorage.setItem("pi-xp-v1",String(Math.max(0,Number(data.totalXp)||0)))}
